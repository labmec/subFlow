...
public:
  ...
  public:
    ...
    /**
   * @brief Class that stores Filter Cake properties.
   */
  class TFilterCakeProperties : public TSFSavable {
  public:
    // associates with a material id the name of the filter cake
    std::map<std::string, int> fCakeNameAndMatId;

    // associates with a material id the density of the solid suspensions
    std::map<int, REAL> fDensity;

    // associates with a material id the mass concentration of the solid suspensions
    std::map<int, REAL> fC;

    // associates with a material id the porosity of the filter cake
    std::map<int, REAL> fPorosity;

    // associates with a material id the mean particle diameter of the solid suspensions
    std::map<int, REAL> fParticleDiameter;

    // associates with a material id the spurt loss time of the filter cake
    std::map<int, REAL> fSpurtLossTime;

    // associates with a material id the spurt loss volume of the filter cake
    std::map<int, REAL> fSpurtLossVolume;

    // Maybe define here function pointers to compute permeability through different models, e.g. Carman-Kozeny

    /** @brief Default constructor */
    TFilterCakeProperties() {}

    /** @brief Destructor */
    ~TFilterCakeProperties() {}

    /** @brief Copy constructor */
    TFilterCakeProperties(const TFilterCakeProperties &other) {
      fCakeNameAndMatId = other.fCakeNameAndMatId;
      fDensity = other.fDensity;
      fC = other.fC;
      fPorosity = other.fPorosity;
      fParticleDiameter = other.fParticleDiameter;
    }

    /** @brief Copy assignment operator*/
    TFilterCakeProperties &operator=(const TFilterCakeProperties &other) {
      fCakeNameAndMatId = other.fCakeNameAndMatId;
      fDensity = other.fDensity;
      fC = other.fC;
      fPorosity = other.fPorosity;
  ...
  TFilterCakeProperties fTFilterCakeProperties;
